...
// GetChannelKey 验证 2FA 后获取渠道密钥 - 已注释，现在直接通过 GetChannel 接口返回密钥
/*
func GetChannelKey(c *gin.Context) {
	...
	if err != nil {
		common.ApiError(c, fmt.Errorf("渠道 ID 格式错误：%v", err))
		return
	}
	...
	if err != nil {
		common.ApiError(c, fmt.Errorf("获取 2FA 信息失败：%v", err))
		return
	}

	if twoFA == nil || !twoFA.IsEnabled {
		common.ApiError(c, fmt.Errorf("用户未启用 2FA，无法查看密钥"))
		return
	}
	...
	// 记录操作日志
	model.RecordLog(userId, model.LogTypeSystem, fmt.Sprintf("查看渠道密钥信息 (渠道 ID: %d)", channelId))
...
// validateTwoFactorAuth 统一的 2FA 验证函数
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if channel.Type == constant.ChannelTypeVertexAi {
		...
		if err != nil {
			return fmt.Errorf("部署地区必须是标准的 Json 格式，例如{\"default\": \"us-central1\", \"region2\": \"us-east1\"}")
		}

		if regionMap["default"] == nil {
			return fmt.Errorf("部署地区必须包含 default 字段")
		}
	}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("批量添加 Vertex AI 必须使用标准的 JsonArray 格式，例如 [{key1}, {key2}...]，请检查输入：%w", err)
	}
...
type PatchChannel struct {
	...
	KeyMode      *string `json:"key_mode"` // 多 key 模式下密钥覆盖或者追加
}
