...
import {
  ...
  add,
  calculateRatioPrice,
  calculateCompletionRatioPrice,
  calculateCacheRatioPrice,
  calculateImageRatioPrice,
...
export function renderQuotaNumberWithDigit(num, digits = 2) {
  ...
  // 对于钱包金额，不做任何取舍，保持完整精度
  // 使用足够的小数位数来显示完整精度
  const fullPrecisionResult = num.toFixed(6);
  let displayResult = parseFloat(fullPrecisionResult).toString();

  // 确保至少有指定的小数位数
  if (!displayResult.includes('.')) {
    displayResult += '.' + '0'.repeat(digits);
  } else {
    const parts = displayResult.split('.');
    if (parts[1].length < digits) {
      displayResult = parts[0] + '.' + parts[1].padEnd(digits, '0');
    }
  }
...
export function renderQuota(quota, digits = 2) {
  let quotaPerUnit = localStorage.getItem('quota_per_unit');
  let displayInCurrency = localStorage.getItem('display_in_currency');
  quotaPerUnit = parseFloat(quotaPerUnit) || 1; // 默认值为1，避免除零错误
  displayInCurrency = displayInCurrency === 'true';
  if (displayInCurrency) {
    if (!quota && quota !== 0) {
        return '¥0.00';
    ...
    const quotaDecimal = new Decimal(quota);
    const quotaPerUnitDecimal = new Decimal(quotaPerUnit);
    const result = quotaDecimal.div(quotaPerUnitDecimal);

    // 对于钱包金额，不做任何取舍，保持完整精度
    // 使用足够的小数位数来显示完整精度，避免精度丢失
    const fullPrecisionResult = result.toFixed(6); // 使用6位小数确保精度
    ...
    if (!displayResult.includes('.')) {
      ...
      const parts = displayResult.split('.');
      if (parts[1].length < 2) {
        displayResult = parts[0] + '.' + parts[1].padEnd(2, '0');
      }
    }
  ...
  if (modelPrice !== -1) {
    return i18next.t(
      '模型价格：¥{{price}} * {{ratioType}}：{{ratio}} = ¥{{total}}',
      {
        price: modelPrice,
        ratio: groupRatio,
        total: multiply(modelPrice, groupRatio),
        ratioType: ratioLabel,
      },
    );
  } else {
    ...
    let inputRatioPrice = calculateRatioPrice(modelRatio);
    let completionRatioPrice = calculateCompletionRatioPrice(modelRatio, completionRatio);
    let cacheRatioPrice = calculateCacheRatioPrice(modelRatio, cacheRatio);
    let imageRatioPrice = calculateImageRatioPrice(modelRatio, imageRatio);
