...
export const calculateModelPrice = ({
  ...
  if (record.quota_type === 0) {
    // 按量计费
    const inputRatioPriceUSD = multiply(multiply(record.model_ratio, 2), usedGroupRatio);
    const completionRatioPriceUSD = multiply(
      multiply(multiply(record.model_ratio, record.completion_ratio), 2),
      usedGroupRatio
    );
    ...
    const numInput = divide(
      parseFloat(rawDisplayInput.replace(/[^0-9.]/g, '')),
      unitDivisor
    );
    const numCompletion = divide(
      parseFloat(rawDisplayCompletion.replace(/[^0-9.]/g, '')),
      unitDivisor
    );

    return {
      inputPrice: `¥${Number(numInput).toFixed(precision)}`,
      completionPrice: `¥${Number(numCompletion).toFixed(precision)}`,
      unitLabel,
      isPerToken: true,
      usedGroup,
      usedGroupRatio,
