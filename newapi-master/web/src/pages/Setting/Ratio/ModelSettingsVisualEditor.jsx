...
import { API, showError, showSuccess, getQuotaPerUnit, multiply, divide } from '../../../helpers';
...
export default function ModelSettingsVisualEditor(props) {
  ...
  const columns = [
    ...
    },
    {
      title: (
        <span>
          {t('模型倍率')}
          <Tooltip content={t('输入价格 = 2 × 模型倍率')}>
            <IconHelpCircle style={{ marginLeft: 4, cursor: 'help' }} />
          </Tooltip>
        </span>
      ),
      dataIndex: 'ratio',
    ...
    },
    {
      title: (
        <span>
          {t('补全倍率')}
          <Tooltip content={t('输出价格 = 输入价格 × 补全倍率')}>
            <IconHelpCircle style={{ marginLeft: 4, cursor: 'help' }} />
          </Tooltip>
        </span>
      ),
  ...
  const calculateCompletionRatioFromPrices = (
    ...
    return divide(completionTokenPrice, modelTokenPrice);
  };
  ...
  const editModel = (record) => {
    ...
    if (record.ratio) {
      ...
      if (record.completionRatio) {
        modelCopy.completionTokenPrice = multiply(
          modelCopy.tokenPrice,
        ...
        onOk={() => {
          if (currentModel) {
            ...
            if (
              ...
              valuesToSave.ratio = divide(currentModel.tokenPrice, 2);
              ...
              if (
                ...
                if (modelPrice > 0) {
                  valuesToSave.completionRatio = divide(
                    currentModel.completionTokenPrice,
                    currentModel.tokenPrice
                  );
                }
