...
const OtherSetting = () => {
  ...
  const submitICPNumber = async () => {
    try {
      setLoadingInput((loadingInput) => ({ ...loadingInput, ICPNumber: true }));
      await updateOption('ICPNumber', inputs.ICPNumber);
      showSuccess(t('备案号已更新'));
    } catch (error) {
      console.error(t('备案号更新失败'), error);
      showError(t('备案号更新失败'));
    } finally {
      setLoadingInput((loadingInput) => ({ ...loadingInput, ICPNumber: false }));
    }
  };
          ...
          <Card>
            <Form.Section text={t('通用设置')}>
              ...
              <Form.Input
                ...
                onChange={handleInputChange}
              />
              <Button onClick={submitICPNumber} loading={loadingInput['ICPNumber']}>
                {t('设置备案号')}
              </Button>
            </Form.Section>
          </Card>
