...
const FooterBar = () => {
  ...
  const [icpNumber, setIcpNumber] = useState(localStorage.getItem('icp_number') || '');
  ...
  const customFooter = useMemo(
    () => (
      <footer className='relative h-auto py-16 px-6 md:px-24 w-full flex flex-col items-center justify-between overflow-hidden'>
        ...
        <div className='flex flex-col items-center justify-center w-full max-w-[1110px] gap-6'>
          {icpNumber && (
    ...
    [logo, systemName, t, currentYear, isDemoSiteMode, icpNumber],
  );
  ...
  useEffect(() => {
    // 从状态或 localStorage 获取备案号
    const icp = statusState?.status?.icp_number || localStorage.getItem('icp_number') || '';
    setIcpNumber(icp);
  }, [statusState]);
