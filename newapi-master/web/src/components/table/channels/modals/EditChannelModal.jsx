...
const EditChannelModal = (props) => {
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      // 在编辑模式下，不显示实际的密钥内容，使用占位符
      const displayData = { ...data };
      if (isEdit && data.key) {
        displayData.key = '••••••••••••••••'; // 使用占位符隐藏实际密钥
      }

      setInputs(displayData);
      if (formApiRef.current) {
        formApiRef.current.setValues(displayData);
      }
      if (data.auto_ban === 0) {
        setAutoBan(false);
      } else {
  ...
  // 显示 2FA 验证模态框 - 使用 TwoFactorAuthModal
  ...
  }, [originModelOptions, inputs.models, isEdit, t]);
  ...
  const submit = async () => {
    ...
    // 如果是编辑模式且 key 为空字符串或占位符，避免提交空值覆盖旧密钥
    if (isEdit && (!localInputs.key || localInputs.key.trim() === '' || localInputs.key === '••••••••••••••••')) {
      delete localInputs.key;
    }
  ...
  return (
    <>
              ...
              <div className='p-2'>
                      ...
                      inputs.type !== 45 && (
                        <div>
                          <Form.Input
                            field='base_url'
                            label={t('API地址')}
                            rules={[{ required: true, message: t('请为渠道填写API地址') }]}
                            placeholder={t(
                ...
                {/* <Card className='!rounded-2xl shadow-sm border-0 mb-6'>
                  ...
                  {/* <JSONEditor
                    ...
                    placeholder={
                      t(
                        '此项可选，用于复写返回的状态码，仅影响本地判断，不修改返回到上游的状态码，比如将 claude 渠道的 400 错误复写为 500（用于重试），请勿滥用该功能，例如：',
                      ) +
                      '\n' +
      ...
      {/* 使用 TwoFactorAuthModal 组件进行 2FA 验证 */}
      <TwoFactorAuthModal
