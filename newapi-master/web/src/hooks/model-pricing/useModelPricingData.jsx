...
export const useModelPricingData = () => {
  ...
  const setModelsFormat = (models, groupRatio, vendorMap) => {
    for (let i = 0; i < models.length; i++) {
      const m = models[i];
      m.key = m.model_name;
      // 对于有enable_groups的模型，使用enable_groups中的分组倍率
      if (m.enable_groups && m.enable_groups.length > 0) {
        // 找到enable_groups中倍率最小的分组
        let minRatio = Number.POSITIVE_INFINITY;
        let selectedGroup = null;
        m.enable_groups.forEach(group => {
        ...
        m.group_ratio = selectedGroup ? groupRatio[selectedGroup] : 1;
      } else {
        m.group_ratio = groupRatio[m.model_name] || 1;
      }
