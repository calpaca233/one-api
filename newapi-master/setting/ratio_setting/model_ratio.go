...
func getHardcodedCompletionModelRatio(name string) (float64, bool) {
	...
	if strings.HasPrefix(name, "gemini-") {
		if strings.HasPrefix(name, "gemini-1.5") {
			...
			return 4, true
		} else if strings.HasPrefix(name, "gemini-2.5-pro") { // 移除 preview 来增加兼容性，这里假设正式版的倍率和 preview 一致
			return 8, false
		} else if strings.HasPrefix(name, "gemini-2.5-flash") { // 处理不同的 flash 模型倍率
	...
	// hint 只给官方上 4 倍率，由于开源模型供应商自行定价，不对其进行补全倍率进行强制对齐
